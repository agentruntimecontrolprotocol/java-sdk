...
/**
 * §4.2 stdio transport round-trip: parent JVM hosts both peers wired through a {@link QueuePipe}
 * that mirrors a child process's stdin/stdout without the thread-affinity pitfalls of {@code
...
class StdioRoundTripTest {
  ...
  void runtimeAndClientExchangeOverPipes() throws Exception {
    ...
    Transport runtimeTransport = new StdioTransport(pipe.aIn, pipe.aOut).start();
    Transport clientTransport = new StdioTransport(pipe.bIn, pipe.bOut).start();
      ...
      JobHandle handle =
          client.submit(
              ArcpClient.jobSubmit(
                  "stdio-echo@1.0.0", JsonNodeFactory.instance.objectNode().put("ping", "pong")));
