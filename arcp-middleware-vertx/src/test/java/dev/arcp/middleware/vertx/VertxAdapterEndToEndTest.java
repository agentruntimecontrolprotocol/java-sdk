...
class VertxAdapterEndToEndTest {
  ...
  void adapterAcceptsAndExecutesJob() throws Exception {
    ArcpRuntime runtime =
        ArcpRuntime.builder()
            .agent(
                "vertx-echo", "1.0.0", (input, ctx) -> JobOutcome.Success.inline(input.payload()))
            .build();
    ...
    try {
      ...
      HttpServer server =
          vertx.createHttpServer(new HttpServerOptions().setPort(0)).webSocketHandler(handler);
      CompletableFuture<Integer> portFuture = new CompletableFuture<>();
      server
          .listen()
          .onSuccess(http -> portFuture.complete(http.actualPort()))
