...
/**
 * §9.6 budget arithmetic property: a sequence of decrements never silently loses precision; the
 * running counter equals {@code initial - sum(decrements)} exactly. {@code BigDecimal} subtraction
 * is associative — the test sweeps adversarial inputs (0.1, 0.2 etc) where {@code double}
...
class BudgetArithmeticPropertyTest {
  ...
  boolean decrementsAreExactBigDecimal(
      @ForAll("initial") BigDecimal initial, @ForAll("decrements") List<BigDecimal> decrements) {
    ...
    BigDecimal observed = counters.remaining("USD").setScale(6, RoundingMode.HALF_UP);
    return observed.compareTo(expected.setScale(6, RoundingMode.HALF_UP)) == 0;
  }

  @Property
  boolean negativeDecrementsAreSilentlyDropped(
      @ForAll("initial") BigDecimal initial, @ForAll("negatives") BigDecimal negative) {
    BudgetCounters counters = new BudgetCounters(Map.of("USD", initial));
