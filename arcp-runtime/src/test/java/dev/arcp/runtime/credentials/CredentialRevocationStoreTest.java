...
class CredentialRevocationStoreTest {
  ...
  void inMemoryStoreTracksOutstandingCredentials() {
    ...
    assertThat(store.outstanding())
        .containsExactly(new CredentialRevocationStore.Outstanding(id, "handle-1"));
  ...
  void fileStoreReplaysOutstandingCredentials(@TempDir Path dir) {
    ...
    assertThat(reloaded.outstanding())
        .containsExactly(new CredentialRevocationStore.Outstanding(second, "handle-2"));
  }
}
