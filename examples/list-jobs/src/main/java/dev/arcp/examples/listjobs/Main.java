  ...
  public static void main(String[] args) throws Exception {
    ...
    ArcpRuntime runtime =
        ArcpRuntime.builder()
            ...
            .agent("fast", "1.0.0", (input, ctx) -> JobOutcome.Success.inline(input.payload()))
            .build();
    ...
    try (ArcpClient client = ArcpClient.builder(pair.client()).build()) {
      ...
      var fast =
          client.submit(ArcpClient.jobSubmit("fast@1.0.0", JsonNodeFactory.instance.objectNode()));
      fast.result().get(5, TimeUnit.SECONDS);

      client.submit(ArcpClient.jobSubmit("blocker@1.0.0", JsonNodeFactory.instance.objectNode()));

      Page<JobSummary> all = client.listJobs(JobFilter.all());
      assert all.items().size() == 2 : "expected 2 jobs, got " + all.items().size();

      Page<JobSummary> running =
          client.listJobs(new JobFilter(java.util.List.of("running"), null, null));
      assert running.items().size() == 1 : "expected 1 running, got " + running.items().size();
