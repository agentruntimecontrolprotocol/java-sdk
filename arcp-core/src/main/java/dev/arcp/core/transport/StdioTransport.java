 ...
 * peers construct a {@link StdioTransport} over their respective stream pair.
 *
 * <p>Framing rules:
 *
 * <ul>
 *   <li>Outbound: JSON is written on one line (Jackson default, no embedded newlines) followed by
 *       {@code \n}.
 *   <li>Inbound: a virtual-thread reader pulls one line at a time and parses it as an {@link
 *       Envelope}. Malformed lines are dropped with a WARN.
 *   <li>End-of-stream terminates the inbound publisher.
 *   <li>{@code stderr} is reserved for log output by convention; this transport never reads or
 *       writes it.
 * </ul>
 */
public final class StdioTransport implements Transport {
  ...
  public StdioTransport(InputStream in, OutputStream out, ObjectMapper mapper) {
    ...
    this.writer =
        new BufferedWriter(
            new OutputStreamWriter(Objects.requireNonNull(out, "out"), StandardCharsets.UTF_8));
    this.reader =
        new BufferedReader(
