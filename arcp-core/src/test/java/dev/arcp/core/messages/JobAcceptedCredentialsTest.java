...
class JobAcceptedCredentialsTest {
  ...
  void credentialsAreOptionalOnJobAccepted() throws Exception {
    ...
    assertThat(
            ArcpMapper.shared()
                .readTree(ArcpMapper.shared().writeValueAsString(withCredentials))
                .has("credentials"))
        .isTrue();

    JobAccepted withoutCredentials =
        new JobAccepted(
            JobId.of("job_2"),
            "agent@1.0.0",
            Lease.empty(),
    ...
    assertThat(
            ArcpMapper.shared()
                .readTree(ArcpMapper.shared().writeValueAsString(withoutCredentials))
                .has("credentials"))
        .isFalse();
  }
}
