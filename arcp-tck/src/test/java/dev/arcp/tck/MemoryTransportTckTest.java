...
/**
 * Runs {@link ConformanceSuite} against a vanilla in-process runtime + paired {@link
 * MemoryTransport}. Self-validation that the harness terminates and that the reference runtime
 * satisfies it.
 */
class MemoryTransportTckTest {
  ...
  private static final class InProcessProvider implements TckProvider {
    private final ArcpRuntime runtime =
        ArcpRuntime.builder()
            .agent("tck-echo", "1.0.0", (input, ctx) -> JobOutcome.Success.inline(input.payload()))
            .agent(
                "tck-log-emitter",
                "1.0.0",
                (input, ctx) -> {
                  ctx.emit(new LogEvent("info", "tck-emit"));
                  return JobOutcome.Success.inline(JsonNodeFactory.instance.objectNode());
                })
            .build();
